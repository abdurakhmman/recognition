...
def recognition5_1(nameid=0, probid=7, testid=0):
    ...
    prob = {}
    for key in freq:
        if ' ' + key[0] in freq_keys: # and key[0] + ' ' in freq_keys:
            prob[key] = freq[key] / freq[' ' + key[0]]  # + freq[key[0] + ' '])
        else:
            prob[key] = 1e-222  # freq[key]
    ...
    for k in keys:
        ...
        for kk in keys:
            if k + kk in freq_keys:
                prob_total[k] += prob[k + kk]
    for key in prob:
        prob[key] = prob[key] / prob_total[key[0]]
    if testid:
        print(freq)
        print(prob)
    ...
    def q(k1, k2, s, x, p, h=height, w=width):
        ...
        if p == 0:
            for i in range(h):
                for j in range(w):
                    bit = x[j + s * w, i] ^ keys_img[k2][j, i]
                    qu += bit * np.log(eps) + (1 ^ bit) * np.log(1 - eps)
        if p == 1:
            for i in range(height):
                for j in range(width):
                    bit = x[j + s * width, i] ^ keys_img[k2][j, i]
                    qu += bit * np.log(1 - eps) + (1 ^ bit) * np.log(eps)
        if k1 + k2 not in freq_keys or prob[k1 + k2] == 0:
            return qu + np.log(eps)  # * 1e5
        if k1 + k2 in freq_keys:
            return qu + np.log(prob[k1 + k2])
